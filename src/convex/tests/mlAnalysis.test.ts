 ...
 * 
 * Note: These are test specifications for documentation purposes.
 * To run actual tests, you would need to set up a testing framework like Jest or Vitest.
 */

// Test specifications (not executable without test framework)
const testSpecs = {
  "ML Analysis Helper Functions": {
    "extractTrackerType": {
      "should identify Analytics tracker": () => {
      const text = "This is an Analytics tracker for monitoring user behavior";
      // Expected: "Analytics"
    },

    "should identify Advertising tracker": () => {
  ...
  "extractRiskLevel": {
    "should identify critical risk level": () => {
  ...
  "extractConfidence": {
    "should extract confidence score from text": () => {
      const text = "Confidence: 85 based on analysis";
      // Expected: 85
    },

    "should return default confidence when not found": () => {
  ...
  "calculateFeatureImportance": {
    "should calculate correct proportions": () => {
      const args = {
        cookieCount: 10,
        trackerCount: 5,
  ...
  "generateRecommendations": {
    "should recommend blocking for high scores": () => {
      const score = 80;
      const level = "high";
    ...
    "should provide default recommendation for low scores": () => {
      const score = 20;
      const level = "low";
...
"ML Analysis Actions": {
  "classifyTracker": {
    "should classify tracker with ML when API key is available": async () => {
      // Mock: HuggingFace API returns classification
      // Expected: type, riskLevel, confidence, explainability
    },

    "should fallback to rule-based when ML fails": async () => {
  ...
  "calculatePersonalizedRisk": {
    "should calculate risk with user history": async () => {
      // Mock: User with history of high-risk sites
      // Expected: higher risk score
    },

    "should handle new users without history": async () => {
...
"ML Explainability": {
  "explainTrackerClassification": {
    "should provide explanation with factors": async () => {
      // Expected: decision, confidence, reasoning array, alternatives
    },

    "should calculate confidence based on risk level": () => {
      // critical: 0.95, high: 0.85, medium: 0.70, low: 0.60
    },
  },

  "explainRiskScore": {
    "should explain risk score with factors": async () => {
